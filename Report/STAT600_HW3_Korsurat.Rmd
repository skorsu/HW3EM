...
We begin by running the EM algorithm. I have set the starting values as $\boldsymbol{\theta}{0} = \left(p = 0.25, \lambda = 0.5, \mu = 2.5\right)$. The stopping criterion for the EM algorithm is $||\boldsymbol{\theta}^{(t+1)} - \boldsymbol{\theta}^{(t)}||{2} < 1 \times 10^{-5}$, where $||\cdot||_{2}$ denotes the Euclidean distance.
...
VARBoots <- foreach(t = 1:100, .combine = "rbind") %dopar% {
  ...
  bootMat <- BootResult(y = simDat[[t]], p0 = 0.2, lambda0 = 0.5, mu0 = 2.5, 
                        eps = 1e-5, EMinit = resultEM[t, ], M = 1000)
  apply(bootMat, 2, var)
...
Tables 2 and 3 show the standard error calculations based on Louis's method and the bootstrap, respectively. Note that for the bootstrap method, I have set M (the number of resample sampling) = 1000. We notice that Louis's method provided the higher standard error compared to the bootstrap. This leads to the conclusion that the coverage probability provided by Louis's method is higher than the one with the bootstrap. Note that the 95% confidence interval is calculated as $\hat{\theta_{j}} \pm z_{0.05/2}\text{SE}\left(\hat{\theta_{j}}\right)$.
